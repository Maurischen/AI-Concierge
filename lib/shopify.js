...
function promotedVariantFromAdminProduct(product) {
  return product.variants?.nodes?.find(
    (variant) => Number(variant.inventoryQuantity || 0) > 0 && normalizeVariantPromotion(variant).active
  );
}
...
function promotedVariantFromWebhookProduct(product) {
  return product.variants?.find(
    (variant) => Number(variant.inventory_quantity || 0) > 0 && normalizeVariantPromotion(variant).active
  );
}

function normalizeMetafields(metafields = [], defaultNamespace = null) {
  const list = Array.isArray(metafields) ? metafields : metafields?.nodes || [];
  return list
    .filter(Boolean)
    .map((metafield) => ({
      namespace: metafield.namespace || defaultNamespace,
      key: metafield.key,
...
const promoMetafieldKeys = new Set([
...
function normalizePromoMetafields(metafields = []) {
  return normalizeMetafields(metafields, "custom").filter(
    (metafield) => metafield.namespace === "custom" && promoMetafieldKeys.has(metafield.key)
  );
}

export function normalizeVariantPromotion(variant) {
  const metafields = normalizePromoMetafields(variant?.metafields || []);
...
export function normalizeAdminProduct(product) {
  const variant = promotedVariantFromAdminProduct(product) || variantFromAdminProduct(product);
...
export function normalizeWebhookProduct(product) {
  const variant = promotedVariantFromWebhookProduct(product) || variantFromWebhookProduct(product);
  if (!variant) return null;
