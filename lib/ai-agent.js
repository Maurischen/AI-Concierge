...
function productSummary(product) {
  const productUrl = product.shopDomain && product.handle ? `https://${product.shopDomain}/products/${product.handle}` : product.handle ? `/products/${product.handle}` : null;
  return {
    variantId: product.variantId,
    name: product.name,
    category: product.category,
    vendor: product.vendor,
    sku: product.sku || product.variants?.find((variant) => variant.id === product.variantId)?.sku || null,
    productUrl,
    price: product.price,
    promotion: product.promotion?.active ? product.promotion : null,
    stock: product.stock,
...
function tools() {
  return [
    ...
    description:
        "Search the merchant's live in-stock Shopify catalog. Use this before recommending products. Query should include exact SKU/model codes, product type, compatibility details, size/capacity, connector, brand preference, and budget when known. For promotions, include terms like promotion, sale, special, deal, clearance, or last chance. For networking switches, include terms like network switch, Ethernet, LAN, PoE, port count, managed/unmanaged, and speed.",
      parameters: {
        type: "object",
        properties: {
          query: {
  ...
  const input = [
    {
      role: "system",
      content:
        "You are a practical electronics shopping assistant for a Shopify store. You have general computer hardware knowledge, but you may recommend only products returned by tools from the merchant's live catalog. Use exact SKU/model codes when the customer gives one, including hyphenated codes like TL-SG1008D. If the customer asks for promotions, specials, sales, clearance, discounts, or deals, search for promotion-marked products and recommend only products returned as promotional matches. Never invent a promotion and never call a product discounted unless the returned product has promotion.active true. When recommending a promotion, mention the product title, normal price, discounted price, promo label, SKU, product URL, and promo source such as STANDALONE, BUNDLE, UPSELL, or PRE_LOVED when present. If the customer gives a model they already own and asks whether an accessory will work with it, treat that model as compatibility context and recommend the requested accessory, not the device they already own. For example, if they ask whether a Dell monitor works with a DisplayPort cable, recommend suitable DisplayPort/monitor cables or ask what ports are available; do not recommend the monitor. If they later clarify that the laptop/PC only has HDMI, recommend HDMI-to-HDMI or HDMI-to-DisplayPort cable/adapter options as appropriate, never a monitor. For networking switch requests, recommend only network/Ethernet/LAN/PoE switches, not HDMI switches, KVM switches, routers, adapters, cables, or media converters. Ask a short follow-up question when required compatibility details are missing. For standalone CPU requests, do not recommend laptops, desktops, or complete PCs unless the customer asks for a full computer. If the customer asks for an Intel i3/i5/i7/i9 or AMD CPU without a socket, motherboard model, or platform, ask for that before searching. For RAM upgrades, reason about total capacity, current module layout, slot count, DDR generation, and DIMM/SODIMM form factor. If the customer has 2x16GB and wants 64GB, do not recommend 16GB modules unless they have at least 4 slots and need two more matching sticks. If slot count or motherboard/laptop model is missing, ask for it. Return JSON only."
    },
    {
      role: "user",
