...
function extractRequestedBrands(products, text) {
  const terms = text.toLowerCase();
  const preferenceSegment = terms.match(/\b(?:prefer|preferred|preference)\b(.{0,90})/)?.[1] || "";
  const preferenceBrands = preferenceSegment
    .replace(/\b(brand|brands|make|makes|vendor|vendors|would|like|a|an|the)\b/g, " ")
    .split(/\s+or\s+|\s+and\s+|[,/]/)
    .map((brand) => brand.trim().split(/\s+/).filter(Boolean).slice(-1)[0])
    .filter((brand) => brand && !["screen", "monitor", "display", "budget", "rand", "zar"].includes(brand));
  const brands = [
    ...new Set(
      products
  ...
  const catalogBrands = brands.filter((brand) => {
    ...
    return new RegExp(`\\b${escaped}\\b`, "i").test(terms);
  });

  const preferenceBrandMatches = [...new Set(preferenceBrands)].filter((brand) =>
    products.some((product) => productBrandText(product).includes(brand.toLowerCase()))
  );

  return [...new Set([...catalogBrands, ...preferenceBrandMatches])];
}
...
export function recommendProducts(products, text, options = {}) {
  ...
  if (candidates.length === 0 && !wantsScreen && !ramRequest && !cableRequest && !hasExplicitIntent) {
    candidates = products.filter((product) => product.stock > 0 && genericCatalogRelevanceScore(product, text) >= 8);
  }
