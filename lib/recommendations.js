...
function promotionLabel(product) {
  const sourceValues = [
    ...
    ...(product.tags || []),
    ...(product.collections || []).map((collection) => collection.title),
    ...(product.metafields || []).flatMap((metafield) => [metafield.key, metafield.value])
  ]
    .filter(Boolean)
    .map((value) => String(value).toLowerCase());

  const joined = sourceValues.join(" ");
  if (/\blast chance\b/.test(joined)) return "Last chance";
  if (/\bclearance\b/.test(joined)) return "Clearance";
  if (/\bon promotion\b|\bpromotion\b|\bpromo\b|\bpromotional\b/.test(joined)) return "On promotion";
  if (/\bon special\b|\bspecials?\b/.test(joined)) return "On special";
  if (/\bsale\b|\bdeals?\b|\bdiscount\b/.test(joined)) return "Sale";
  return null;
}

function isPromotionProduct(product) {
  return Boolean(promotionLabel(product));
}
...
export function reasonsFor(product, text, preferredBrands = {}, requestedBrands = []) {
  ...
  if (promo) {
    reasons.push(`${promo} item in the current Shopify catalog.`);
  }
