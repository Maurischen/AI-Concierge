...
export function isRelevantProductForRequest(product, text) {
  ...
  if (!productMatchesRequestedSpecs(product, text)) return false;
...
export function scoreProduct(product, text, preferredBrands = {}) {
  ...
  if (!productMatchesRequestedSpecs(product, text)) score -= 80;
  ...
  if (terms.includes("ddr5") && wantsRam(text) && !searchableProductText.includes("ddr5")) score -= 20;
