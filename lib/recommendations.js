...
function isPromotionRequest(text) {
  return /\b(on promotion|promotion|promo|promotional|specials?|on special|sale|deals?|discount|clearance|last chance)\b/i.test(text);
}

function promotionLabel(product) {
  const promo = product?.promotion;
  if (!promo?.active) return null;
  return promo.label || "Promotion";
}

function isPromotionProduct(product) {
  return product?.promotion?.active === true;
}

function promotionPriority(product) {
  const priority = Number(product?.promotion?.priority);
  return Number.isFinite(priority) && priority > 0 ? priority : Number.MAX_SAFE_INTEGER;
}

function promotionSavings(product) {
  const savings = Number(product?.promotion?.savings || 0);
...
function ramUpgradePlan(text) {
  ...
  const slotMatch =
    terms.match(/\b([2-8])\s*ram\s*slots?\b/) ||
    terms.match(/\b([2-8])\s*(?:memory\s*slots?|slots?|dimms?)\b/) ||
    terms.match(/\b(?:ram\s*slots?|memory\s*slots?|slots?|dimms?)\D{0,8}([2-8])\b/);

  const targetGb = targetMatch ? Number(targetMatch[1]) : null;
  const currentModules = currentMatch ? Number(currentMatch[1]) : null;
  const currentModuleGb = currentMatch ? Number(currentMatch[2]) : null;
  const slotCount = slotMatch ? Number(slotMatch[1]) : null;
  const currentTotalGb = currentModules && currentModuleGb ? currentModules * currentModuleGb : null;

  if (!targetGb && !currentTotalGb && !slotCount) return null;
...
function requestedRamModuleCapacityGb(text) {
  const plan = ramUpgradePlan(text);
  if (!plan?.targetGb || !plan.currentModules || !plan.currentModuleGb || !plan.slotCount) return null;

  if (plan.slotCount <= plan.currentModules) {
    return Math.ceil(plan.targetGb / Math.max(1, plan.slotCount));
  }

  const openSlots = plan.slotCount - plan.currentModules;
  const additionalGb = plan.targetGb - plan.currentTotalGb;
  if (additionalGb <= 0) return null;
  return Math.ceil(additionalGb / openSlots);
}

function productRamCapacityGb(product) {
  const text = productText(product).replace(/(\d)\s+(\d{3})/g, "$1$2");
  const matches = [...text.matchAll(/\b(4|8|16|24|32|48|64|128)\s?gb\b/g)].map((match) => Number(match[1]));
...
function hasProductNeed(text) {
  return /\b(power bank|powerbank|battery pack|portable charger|charger|psu|power supply|power-supply|cable|cord|lead|hub|dock|adapter|adaptor|bag|backpack|sleeve|case|monitor|screen|display|desktop|laptop|ram|memory|dimm|sodimm|ddr3|ddr4|ddr5|ssd|hdd|hard drive|flash drive|graphics card|gpu|video card|motherboard|mainboard|cpu|processor|keyboard|mouse|steering wheel|racing wheel|wheel|headset|speaker|microphone|webcam|router|switch|access point|ups|printer|toner|cartridge|promotion|promo|special|sale|deal|discount|clearance)\b/i.test(text);
}

function budgetIsOptional(text) {
  return /\b(i don'?t know|not sure|no budget|without a budget|any budget|show me|what is in stock|what's in stock|in stock|available|browse|options|upgrade|compatible|work with|works with|for my|i have|promotion|promo|special|sale|deal|discount|clearance|last chance)\b/i.test(text) ||
    /\bfor\s+(?:an?\s+|my\s+)?[a-z0-9 -]+\s+(laptop|notebook|motherboard|graphics card|gpu|desktop|pc)\b/i.test(text);
}
...
function isChargerRequest(text) {
  return /\b(charger|charging brick|power adapter|power adaptor)\b/i.test(text);
}

function isLaptopChargerRequest(text) {
  return isChargerRequest(text) && /\b(laptop|notebook|acer|asus|dell|hp|lenovo|msi|macbook)\b/i.test(text);
}

function productChargerWattage(product) {
  const text = productText(product);
  const values = [...text.matchAll(/\b(\d{2,3})\s?w\b/g)].map((match) => Number(match[1]));
  return values.length > 0 ? Math.max(...values) : null;
}

function isLaptopChargerProduct(product) {
  const text = productText(product);
  const chargerSignal = /\b(charger|power adapter|power adaptor|ac adapter|ac adaptor|power supply|charging brick)\b/.test(text);
  if (!chargerSignal) return false;
  if (/\b(car charger|vehicle charger|phone charger|mobile charger|travel charger|power bank)\b/.test(text)) return false;
  if (/\b(laptop|notebook|macbook|universal adapter|universal adaptor|ac adapter|ac adaptor)\b/.test(text)) return true;
  const watts = productChargerWattage(product);
  return watts !== null && watts >= 45 && /\b(usb[- ]?c|type[- ]?c|pd|power delivery|gan)\b/.test(text);
}

function laptopChargerRequestHasCompatibility(text) {
  return /\b(model|watt|wattage|\d{2,3}\s?w|volt|voltage|\d{2}\s?v|amp|barrel|round pin|usb[- ]?c|type[- ]?c|pin|connector)\b/i.test(text) ||
    /\b(aspire|extensa|nitro|predator|swift|travelmate|spin|vivobook|ideapad|thinkpad|latitude|inspiron|pavilion|probook|pulse|katana)\b/i.test(text);
}

function asksToBrowseAvailableOptions(text) {
  return /\b(show me|what is available|what's available|in stock|available|options|browse)\b/i.test(text);
}

function productConflictsWithLaptopChargerRequest(product) {
  return !isLaptopChargerProduct(product);
}
...
function accessoryConflict(product, text) {
  if (isLaptopChargerRequest(text)) return null;
  if (isNetworkSwitchRequest(text)) return null;
...
function isMouseRequest(text) {
  return /\b(mouse|mice)\b/i.test(text) && !/\b(mouse\s*pad|mousepad|pad)\b/i.test(text);
}

function isMouseProduct(product) {
  const text = productText(product);
  return /\b(mouse|mice)\b/.test(text) && !/\b(mouse\s*pad|mousepad|pad|phone)\b/.test(text);
}
...
function hasCpuCompatibility(text) {
  return /\b(am4|am5|lga\s?\d+|skt\s?\d+|socket\s?\d+|motherboard|mainboard|board|socket)\b/i.test(text);
}
...
function requestedModelTokens(text) {
  ...
  return [
    ...new Set(
      terms
        .match(/\b[a-z]{1,12}[a-z0-9-]*\d[a-z0-9-]*\b|\b[a-z]{1,8}-[a-z0-9-]{2,}\b|\b\d{3,6}[a-z]?\b/g)
        ?.filter((token) => {
...
function productSkuTokens(product) {
  return [
    product.sku,
    product.handle,
    product.name,
...
function productMatchesRequestedModelTokens(product, text) {
  ...
  const normalizedProductTokens = productSkuTokens(product);
  return tokens.every((token) => {
    const escapedToken = token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const normalizedToken = normalizeSkuToken(token);
    return (
      new RegExp(`\\b${escapedToken}\\b`, "i").test(searchableProductText) ||
      normalizedProductTokens.some((productToken) => productToken.includes(normalizedToken))
    );
  });
}

function isNetworkSwitchRequest(text) {
  const terms = String(text || "").toLowerCase();
  return /\b(network|ethernet|lan|poe|gigabit|10\/100|10\/100\/1000|tp[- ]?link|d[- ]?link|ubiquiti|mikrotik|\d+\s*port)\b/.test(terms) && /\bswitch\b/.test(terms);
}

function requestedPortCount(text) {
  const match = String(text || "").toLowerCase().match(/\b(\d{1,2})\s*[- ]?\s*(?:port|ports)\b/);
  return match ? Number(match[1]) : null;
}

function productPortCount(product) {
  const text = productText(product);
  const matches = [...text.matchAll(/\b(\d{1,2})\s*[- ]?\s*(?:port|ports)\b/g)];
  if (matches.length === 0) return null;
  return Math.max(...matches.map((match) => Number(match[1])));
}

function isNetworkSwitchProduct(product) {
  const text = productText(product);
  const category = String(product.category || "").toLowerCase();
  const title = String(product.name || "").toLowerCase();
  const specs = [...(product.specs || []), ...(product.variants || []).map((variant) => variant.sku)].join(" ").toLowerCase();
  const categoryAndTitle = `${category} ${title}`;
  const coreText = `${categoryAndTitle} ${specs}`;
  const hasSwitchSignal = /\bswitch(es)?\b/.test(categoryAndTitle) || /\b\d{1,2}\s*[- ]?\s*ports?\b/.test(title);
  const hasNetworkSignal = /\b(network|networking|ethernet|lan|poe|gigabit|10\/100|10\/100\/1000|tp[- ]?link|d[- ]?link|ubiquiti|mikrotik)\b/.test(coreText);
  const accessoryConflict = /\b(hdmi|kvm|splitter|selector|converter|adapter|adaptor|cable|media converter)\b/.test(coreText);
  const routerConflict = /\brouter\b/.test(categoryAndTitle);
  const cameraConflict = /\b(camera|cctv|surveillance|security camera|dvr|nvr)\b/.test(categoryAndTitle);
  return hasSwitchSignal && hasNetworkSignal && !accessoryConflict && !routerConflict && !cameraConflict;
}

function isKnownProductTypeRequest(text) {
  return (
    hasProductNeed(text) ||
    isPromotionRequest(text) ||
...
function requestedSuggestionType(text) {
  ...
  if (isNetworkSwitchRequest(terms)) return /\b(network|ethernet|lan|poe|gigabit|switch)\b/;
...
export function isRelevantProductForRequest(product, text) {
  ...
  const wantsScreen = !cableRequest && (terms.includes("screen") || terms.includes("display") || terms.includes("monitor"));
  const laptopChargerRequest = isLaptopChargerRequest(text);
  const psuRequest = isPsuRequest(text);
  const monitorSize = requestedMonitorSize(text);
  if (isPromotionRequest(text) && !isPromotionProduct(product)) return false;
  ...
  if (isMouseRequest(text) && !isMouseProduct(product)) return false;
  if (!cableRequest && requestedModelTokens(text).length > 0 && !productMatchesRequestedModelTokens(product, text)) return false;
  if (isNetworkSwitchRequest(text) && !isNetworkSwitchProduct(product)) return false;
  if (isNetworkSwitchRequest(text) && requestedPortCount(text)) {
    const productPorts = productPortCount(product);
    if (!productPorts || productPorts !== requestedPortCount(text)) return false;
  }
  ...
  if (ramRequest && requestedRamModuleCapacityGb(text)) {
    const productCapacity = productRamCapacityGb(product);
    if (!productCapacity || Math.abs(productCapacity - requestedRamModuleCapacityGb(text)) > 1) return false;
  }
  if (isCpuRequest(text) && !/\b(laptop|notebook|desktop pc|prebuilt|all-in-one|aio|computer)\b/i.test(text)) {
    if (/\blaptop\b|\bnotebook\b|\bdesktop pc\b|\ball-in-one\b|\baio\b/.test(productText(product))) return false;
  }
  if (cableRequest && !isCableProduct(product)) return false;
  if (laptopChargerRequest && productConflictsWithLaptopChargerRequest(product)) return false;
  ...
  if (!cableRequest && !productMatchesRequestedIntents(product, text)) return false;
  if (detectRequestedIntents(text).length === 0 && !isPromotionRequest(text) && genericCatalogRelevanceScore(product, text) < 8) return false;
  if (detectRequestedIntents(text).length === 0 && isPromotionRequest(text) && !isPromotionProduct(product)) return false;
...
export function scoreProduct(product, text, preferredBrands = {}) {
  ...
  if ((terms.includes("home") || terms.includes("wifi")) && product.tags.includes("home") && !isNetworkSwitchRequest(text)) score += 4;
  if (isPromotionRequest(text) && isPromotionProduct(product)) score += 35;
  if (isPromotionRequest(text) && !isPromotionProduct(product)) score -= 80;
  if (isMouseRequest(text) && isMouseProduct(product)) score += 26;
  if (isMouseRequest(text) && !isMouseProduct(product)) score -= 80;
  if (requestedModelTokens(text).length > 0 && productMatchesRequestedModelTokens(product, text)) score += 90;
  if (isNetworkSwitchRequest(text) && isNetworkSwitchProduct(product)) score += 45;
  if (isNetworkSwitchRequest(text) && !isNetworkSwitchProduct(product)) score -= 90;
  if (isNetworkSwitchRequest(text) && requestedPortCount(text)) {
    const productPorts = productPortCount(product);
    if (productPorts === requestedPortCount(text)) score += 24;
    if (productPorts && productPorts !== requestedPortCount(text)) score -= 35;
  }
  ...
  if (isLaptopChargerRequest(text) && isLaptopChargerProduct(product)) score += 32;
  if (isLaptopChargerRequest(text) && !isLaptopChargerProduct(product)) score -= 90;
  ...
  const requestedModuleCapacity = requestedRamModuleCapacityGb(text);
  const productRamCapacity = productRamCapacityGb(product);
  if (requestedModuleCapacity && wantsRam(text)) {
    if (productRamCapacity === requestedModuleCapacity) score += 28;
    if (productRamCapacity && productRamCapacity !== requestedModuleCapacity) score -= 35;
  }
  if (!wantsCable(text)) {
  ...
  if (budgetMatch) {
    ...
    if (effectiveProductPrice(product) <= budget) score += 4;
    if (effectiveProductPrice(product) > budget) score -= 5;
  }
...
export function reasonsFor(product, text, preferredBrands = {}, requestedBrands = []) {
  const terms = text.toLowerCase();
  const reasons = [];
  const modelTokens = requestedModelTokens(text);

  if (modelTokens.length > 0 && productMatchesRequestedModelTokens(product, text)) {
    reasons.push(`Matches requested model/SKU ${modelTokens.join(", ").toUpperCase()}.`);
  }
  if (isNetworkSwitchRequest(text) && isNetworkSwitchProduct(product)) {
    reasons.push("Matches the requested network/Ethernet switch product type.");
  }
  if (isNetworkSwitchRequest(text) && requestedPortCount(text) && productPortCount(product) === requestedPortCount(text)) {
    reasons.push(`Matches the requested ${requestedPortCount(text)}-port switch requirement.`);
  }
  const promo = promotionLabel(product);
  if (promo) {
    const source = product.promotion?.source ? ` (${product.promotion.source})` : "";
    reasons.push(`${promo}${source}: was ${money(product.promotion.normalPrice)}, now ${money(product.promotion.discountedPrice)}.`);
  }
  ...
  const requestedModuleCapacity = requestedRamModuleCapacityGb(text);
  if (wantsRam(text) && isRamProduct(product) && requestedModuleCapacity) {
    reasons.push(`Matches the required ${requestedModuleCapacity}GB module size for the upgrade path.`);
  }
  if (wantsCable(text) && isCableProduct(product)) {
    reasons.push("Matches a cable request, not a charger or power adapter.");
  }
  if (isLaptopChargerRequest(text) && isLaptopChargerProduct(product)) {
    reasons.push("Matches a laptop charger request, not a phone or car charger.");
  }
...
export function recommendProducts(products, text, options = {}) {
  ...
  let candidates = products.filter((product) => {
    ...
    if (budget && effectiveProductPrice(product) > budget && !wantsBundle) return false;
    return true;
  });
  ...
  if (candidates.length === 0 && laptopChargerRequest) {
    candidates = products.filter((product) => product.stock > 0 && isLaptopChargerProduct(product) && isRelevantProductForRequest(product, text));
  }
  ...
  if (candidates.length === 0 && !wantsScreen && !ramRequest && !cableRequest && !laptopChargerRequest && !hasExplicitIntent) {
  ...
  return candidates
    ...
    .sort((a, b) => {
      if (isPromotionRequest(text)) {
        return (
          b.score - a.score ||
          promotionPriority(a) - promotionPriority(b) ||
          promotionSavings(b) - promotionSavings(a) ||
          b.stock - a.stock ||
          effectiveProductPrice(a) - effectiveProductPrice(b)
        );
      }
      return b.score - a.score || effectiveProductPrice(a) - effectiveProductPrice(b);
    })
...
export function getQualificationQuestions(text) {
  const questions = [];
  const ramPlan = ramUpgradePlan(text);

  if (
    wantsRam(text) &&
    /\b(upgrade|upgrading|increase|target total|current ram|currently|installed|already|2x|4x)\b/i.test(text) &&
    ramPlan?.targetGb &&
    ramPlan.currentModules &&
    ramPlan.currentModuleGb &&
    !ramPlan.slotCount
  ) {
    questions.push(
      `How many RAM slots does the PC or motherboard have, and what is the motherboard model if you know it? With ${ramPlan.currentModules}x${ramPlan.currentModuleGb}GB installed, a ${ramPlan.targetGb}GB upgrade may mean replacing the kit or adding matching sticks depending on the slot count.`
    );
  }

  if (isLaptopChargerRequest(text) && !laptopChargerRequestHasCompatibility(text) && !asksToBrowseAvailableOptions(text)) {
    questions.push("What laptop model, charger wattage, and connector type do you need, for example 45W/65W/90W, USB-C, or round barrel pin?");
  }
