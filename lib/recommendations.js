...
export function scoreProduct(product, text) {
  ...
  if ((terms.includes("monitor") || terms.includes("screen") || terms.includes("display")) && searchableProductText.includes("monitor")) score += 8;
...
export function recommendProducts(products, text) {
  ...
  let candidates = products.filter((product) => {
    ...
    if (wantsScreen && !wantsBundle && ![product.name, product.category, product.description, ...(product.tags || []), ...(product.specs || [])].join(" ").toLowerCase().includes("monitor")) return false;
