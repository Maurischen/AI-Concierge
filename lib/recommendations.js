import { productIntentScore, productMatchesRequestedIntents, requestedIntentNames } from "./product-intents.js";
...
function hasProductNeed(text) {
  return /\b(power bank|powerbank|battery pack|portable charger|charger|cable|cord|lead|hub|dock|adapter|adaptor|bag|backpack|sleeve|case|monitor|screen|display|desktop|laptop|ram|memory|dimm|sodimm|ddr3|ddr4|ddr5|ssd|hdd|hard drive|flash drive|graphics card|gpu|video card|motherboard|mainboard|cpu|processor|keyboard|mouse|headset|speaker|microphone|webcam|router|switch|access point|ups|printer|toner|cartridge)\b/i.test(text);
}
...
export function scoreProduct(product, text, preferredBrands = {}) {
  ...
  if (monitorSize && productSize) {
    if (productSize >= monitorSize - 0.3) score += Math.max(0, 12 - Math.abs(productSize - monitorSize));
...
export function recommendProducts(products, text, options = {}) {
  ...
  if (candidates.length === 0 && !wantsScreen && !ramRequest && !cableRequest) {
