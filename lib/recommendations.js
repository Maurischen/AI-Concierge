...
function isNetworkSwitchProduct(product) {
  const text = productText(product);
  const category = String(product.category || "").toLowerCase();
  const hasSwitchSignal = /\bswitch(es)?\b/.test(text);
  const hasNetworkSignal = /\b(network|networking|ethernet|lan|poe|gigabit|10\/100|10\/100\/1000|tp[- ]?link|d[- ]?link|ubiquiti|mikrotik)\b/.test(text);
  const accessoryConflict = /\b(hdmi|kvm|splitter|selector|converter|adapter|adaptor|cable|media converter)\b/.test(text);
  const routerConflict = /\brouter\b/.test(category) || /\brouter\b/.test(String(product.name || "").toLowerCase());
  return hasSwitchSignal && hasNetworkSignal && !accessoryConflict && !routerConflict;
}
