...
export async function listShops() {
  const pool = await ensureCatalogTable();
  if (!pool) return [];
  const result = await pool.query("select shop_domain, storefront_name, market_type, assistant_name, theme_color, logo_url, support_email, sales_email, preferred_brands from shops order by shop_domain");
