...
async function ensureCatalogTable() {
  ...
  await pool.query(`
    create table if not exists catalog_products (
      shop_domain text not null default 'demo',
      id text not null,
      data jsonb not null,
      updated_at timestamptz not null default now(),
      primary key (shop_domain, id)
    )
  `);
...
export async function listShops() {
  const pool = await ensureCatalogTable();
  if (!pool) return [];
  const result = await pool.query("select shop_domain, storefront_name, market_type, assistant_name, theme_color from shops order by shop_domain");
